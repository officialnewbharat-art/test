                 ...
                 <div class="max-w-4xl mx-auto animate-fadeIn pb-20 lg:pb-0">
                    ...
                    ${isDisqualified ? `
                        <div class="bg-white rounded-2xl p-6 lg:p-8 border border-slate-200 text-center mt-4 lg:mt-0 shadow-lg">
                            <h3 class="text-lg lg:text-xl font-bold text-slate-900 mb-4">🚫 Security Violation Report</h3>
                    ...
                    ${activeTab === 'qa' && !isDisqualified ? `
                        <div class="space-y-4 lg:space-y-6">
                            <div class="bg-white p-4 lg:p-6 rounded-2xl shadow-sm border border-slate-200 text-sm lg:text-base text-slate-700">
                               <h4 class="font-bold text-lg text-slate-900 mb-2">🎯 Detailed Marking Breakdown (Q&A Analysis):</h4>
...
async function generateScore(transcript, terminationReason) {
    if (terminationReason && terminationReason !== "Completed" && terminationReason !== "Completed (AI spoken conclusion detected)") {
        setTimeout(() => {
            updateState({ 
                result: { rating: 0, feedback: "Interview terminated early due to security or time violation.", passed: false, questions: [], terminationReason },
                step: AppStep.RESULT 
            });
        }, 1500);
        return;
    ...
    try {
        ...
        const prompt = `
            ...
            1. Analyze the technical accuracy, clarity, and communication style.
            2. **STRICT MARKING**: The overall rating (1-10) and per-question ratings (1-10) must be **genuinely reflective** of performance. A score of 7 or above is a pass.
            3. **Question Structure (5 MANDATORY)**: Identify exactly 5 distinct Q&A segments (questions asked by the AI and answered by the candidate). If the transcript has fewer, deduce the best 5. For each question:
               - **question**: The full text of the question asked by the AI.
               - **candidateAnswerSummary**: A concise summary of the candidate's response. Use "[No Answer - Time Limit Exceeded]" for silence penalties.
               - **rating**: An integer score from 1 to 10.
               - **feedback**: **CRITICAL**: Detailed, professional explanation (1-2 sentences) of the rating. **Explicitly state what was missing or incorrect, and why marks were deducted.**
    ...
    appState.silenceInterval = setInterval(() => {
        ...
        if (timeSinceAiFinished > 10000 && !appState.isProcessingTimeout) {
            ...
            if (appState.silenceWarningCount > 2) {
                 ...
                 appState.session.sendRealtimeInput({ text: "System Alert: Candidate is silent. Say exactly: 'Please start answering when you’re ready.'" });
            }
        }
    }, 1000);
})();
